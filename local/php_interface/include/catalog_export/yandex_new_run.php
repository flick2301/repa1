			...
			if ($row['PREVIEW_TEXT'] !== null)
			{
				$row['DESCRIPTION'] = yandex_text2xml(
					...
					true
                            ...
                            $row['DESCRIPTION'] = yandex_text2xml($row['NAME'], true);
                        }
	...
	if (empty($arRunErrors))
	{
		...
		if (is_array($XML_DATA['CURRENCY']))
		{
			foreach ($XML_DATA['CURRENCY'] as $CURRENCY => $arCurData)
			{
				...
				{
					if (CCurrencyRates::ConvertCurrency(1, $CURRENCY, $RUR)) $strTmp .= '<currency id="'.$CURRENCY.'"'
...
if (empty($arRunErrors))
{
	...
	$itemFields = array(
		...
		'CATALOG_AVAILABLE', 'CATALOG_TYPE', 'PROPERTY_DLINA', 'PROPERTY_DIAMETR', 'PROPERTY_KOLICHESTVO_V_UPAKOVKE', 'PROPERTY_TSVET', 'PROPERTY_BREND', 'CATALOG_WEIGHT'
	);
	$offerFields = array(
		'ID', 'IBLOCK_ID', 'IBLOCK_SECTION_ID', 'NAME',
		'PREVIEW_PICTURE', 'PREVIEW_TEXT', 'PREVIEW_TEXT_TYPE', 'DETAIL_PICTURE', 'DETAIL_PAGE_URL', 'PROPERTY_DLINA', 'PROPERTY_DIAMETR', 'PROPERTY_KOLICHESTVO_V_UPAKOVKE', 'PROPERTY_TSVET', 'PROPERTY_BREND', 'CATALOG_WEIGHT'
	);
					...
					foreach ($arYandexFields as $key)
					{
						switch ($key)
						{
							case 'name':
								...
								$itemsContent .= "<name>".yandex_text2xml(str_replace("(Россия)", " ", $row['NAME']), true)."</name>\n";
								break;
							case 'description':
								$itemsContent .= "<description>".yandex_text2xml(str_replace("(Россия)", " ", iconv('cp1251', "UTF-8", $row['DESCRIPTION'])))."</description>\n";
								break;
							case 'param':
								if ($parametricFieldsExist)
								{
							...
							default:
								...
								if ($fieldsExist && isset($fields[$key]))
								{
									...
									unset($value);
                                        ...
                                        if(strpos($row['NAME'], "кг") !== false):
                                            $units = 'кг';
                                        else:
                                            $units = 'шт.';
                                        endif;
                                       
                                        if($row['PROPERTY_TSVET_VALUE']!='')
                                        $itemsContent .= "<param name='".yandex_text2xml("Цвет RAL")."' >".yandex_text2xml($row['PROPERTY_TSVET_VALUE'], true)."</param>\n";
                                        if($row['PROPERTY_DLINA_VALUE']!='')
                                        $itemsContent .= "<param name='".yandex_text2xml("Длина")."' unit='".yandex_text2xml("мм")."' >".yandex_text2xml($row['PROPERTY_DLINA_VALUE'], true)."</param>\n";
                                        if($row['PROPERTY_DIAMETR_VALUE']!='')
                                        $itemsContent .= "<param name='".yandex_text2xml("Диаметр")."' unit='".yandex_text2xml("мм")."'>".yandex_text2xml($row['PROPERTY_DIAMETR_VALUE'], true)."</param>\n";
                                        if($row['PROPERTY_KOLICHESTVO_V_UPAKOVKE_VALUE']!='')
                                        $itemsContent .= "<param name='".yandex_text2xml("Количество")."' unit='".yandex_text2xml($units)."'>".yandex_text2xml($row['PROPERTY_KOLICHESTVO_V_UPAKOVKE_VALUE'], true)."</param>\n";
                                        if($row['CATALOG_WEIGHT']>0)
                                            $itemsContent .= "<weight>".(intval($row['CATALOG_WEIGHT'])/1000)."</weight>\n";
