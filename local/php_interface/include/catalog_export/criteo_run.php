			...
			if ($row['PREVIEW_TEXT'] !== null)
			{
				$row['DESCRIPTION'] = 
					TruncateText(
						...
						? strip_tags(preg_replace_callback("'&[^;]*;'", '', $row['PREVIEW_TEXT']))
						: preg_replace_callback("'&[^;]*;'", '', $row['PREVIEW_TEXT']),
						$options['MAX_DESCRIPTION_LENGTH']
					);
					
			}else{
                            $row['DESCRIPTION'] = strip_tags(preg_replace_callback("'&[^;]*;'", 'yandex_replace_special', $row['NAME']));
                        }
...
if ($firstStep)
{
	...
	if (empty($arRunErrors))
	{
		...
		fwrite($fp, '<title>'.$site['SITE_NAME']."</title>\n");

		
		fwrite($fp, '<link>'.$site['SERVER_NAME']."</link>\n");
                fwrite($fp, "<description>Магазин крепежа «КРЕП-КОМП». Купить метизы, крепежные изделия и инструмент в Москве.</description>\n");
...
if (empty($arRunErrors))
{
	...
	$itemFields = array(
		...
		'CATALOG_AVAILABLE', 'CATALOG_TYPE', 'PROPERTY_BREND', 'PROPERTY_CML2_BAR_CODE'
	);
	$offerFields = array(
		'ID', 'IBLOCK_ID', 'IBLOCK_SECTION_ID', 'NAME',
		'PREVIEW_PICTURE', 'PREVIEW_TEXT', 'PREVIEW_TEXT_TYPE', 'DETAIL_PICTURE', 'DETAIL_PAGE_URL', 'PROPERTY_BREND', 'PROPERTY_CML2_BAR_CODE'
	);
			...
			foreach ($itemIdsList as $id)
			{
				...
				elseif (isset($simpleIdsList[$id]) && !empty($row['PRICES']))
				{
					...
					$available = ' available="'.($row['CATALOG_AVAILABLE'] == 'Y' ? 'true' : 'false').'"';
                                        if($row['CATALOG_AVAILABLE']=='Y' && $row['PICTURE']!=''):
					$itemsContent .= "<item>\n";
					unset($available);
					...
					foreach ($arYandexFields as $key)
					{
						switch ($key)
						{
							case 'name':
								...
								//$itemsContent .= "<title>". strip_tags(preg_replace("/&(?!#?[a-z0-9]+;)/", "&amp;", $row['NAME']))."</title>\n";
								break;
							case 'description':
								//$itemsContent .= "<description>".strip_tags(preg_replace("/&(?!#?[a-z0-9]+;)/", "&amp;", $row['DESCRIPTION']))."</description>\n";
								break;
							case 'param':
								if ($parametricFieldsExist)
								{
							...
							default:
								...
								if ($fieldsExist && isset($fields[$key]))
								{
									...
									unset($value);
										...
										$itemsContent .= "<g:title>". strip_tags(preg_replace("/&(?!#?[a-z0-9]+;)/", "&amp;", $row['NAME']))."</g:title>\n";
										$itemsContent .= "<g:description>".strip_tags(preg_replace("/&(?!#?[a-z0-9]+;)/", "&amp;", $row['DESCRIPTION']))."</g:description>\n";
										
					$referer = '';
					if (!$disableReferers)
						$referer = (strpos($row['DETAIL_PAGE_URL'], '?') === false ? '?' : '&amp;').'r1=<?=$strReferer1; ?>&amp;r2=<?=$strReferer2; ?>';

					$itemsContent .= "<g:link>".$usedProtocol.$site['SERVER_NAME'].htmlspecialcharsbx($row['DETAIL_PAGE_URL'])."</g:link>\n";
					unset($referer);	
					...
					if ($minPrice != $row['PRICES'][NUMBER_BASE_PRICE]['PRICE'] && $minPrice < $row['PRICES'][NUMBER_BASE_PRICE]['PRICE']) {
						$itemsContent .= "<g:price>".$minPrice." RUB</g:price>\n";
						$itemsContent .= "<g:sale_price>".$row['PRICES'][NUMBER_BASE_PRICE]['PRICE']."</g:sale_price>\n";
					}
					else {
						$itemsContent .= "<g:price>".$minPrice." RUB</g:price>\n";
					}
					
					if ($row['PROPERTY_CML2_BAR_CODE_VALUE']) $itemsContent .= "<g:gtin>".$row['PROPERTY_CML2_BAR_CODE_VALUE']."</g:gtin>\n";
					if ($row['PROPERTY_BREND_VALUE']) $itemsContent .= "<g:brand>".$row['PROPERTY_BREND_VALUE']."</g:brand>\n";
