...
(function() {
	...
	BX.Sale.OrderAjaxComponent = {
		...
		refreshOrder: function(result)
		{
			...
			var currentstore = $('input[name=DELIVERY_ID]:checked').parent().attr('id').replace(/[^0-9]+/, '');	
		...
		{
			return; //Закрываем открытие первого блока
			...
			else if (personTypesCount == 2)
			{
				for (i in this.result.PERSON_TYPE)
				{
					if (this.result.PERSON_TYPE.hasOwnProperty(i))
					{
						...
						label = BX.create('LABEL',
						{
							props: {className: 'form__label' + (currentType.CHECKED == 'Y' ? ' active' : '')},
							children: [
								BX.create('DIV', {
									props: {className: 'radio__wrap'},
										children: [
										BX.create('INPUT', {
											attrs: {checked: currentType.CHECKED == 'Y'},
											props: {type: 'radio', className: 'form__radio', name: 'PERSON_TYPE', value: currentType.ID},
										}),
									
									]
								}),
								BX.create('SPAN', {props: {className: 'radio__text'}, html: BX.util.htmlspecialchars(currentType.NAME)}),
							],
							events: {change: BX.proxy(this.sendRequest, this)}
						});
						...
						node.appendChild(BX.create('DIV', {props: {className: 'feedback-form__left label_block'}, children: [label]}));
						delimiter = true;
		...
		createPaySystemItem: function(item)
		{
			var checked = item.CHECKED == 'Y',
				logotype, logoNode, payLogo, payLogoBlock,
...
/*
			...
			logotype = this.getImageSources(item, 'PSA_LOGOTIP');
			if (logotype.src_1x) {
				payLogo = BX.create('IMG', {props: {className: 'payment-logotype'}});
				payLogo.setAttribute('src', logotype.src_1x);
				payLogo.setAttribute('alt', item.NAME);
				payLogo.setAttribute('style', 'position: absolute; height: 25px;');
				
				payLogoBlock = BX.create('div', {
					props: {},
					children: [payLogo],
				}),
				payLogoBlock.setAttribute('style', 'float: right; position: relative; top: -12px; left: 15px;');				
			}	
			
			
			label = BX.create('LABEL', {
				props: {className: 'form__label' + (checked ? ' active' : '')},
				children: [
					BX.create('DIV', {
						props: {className: 'radio__wrap'},
						children: [
			...
			itemNode = BX.create('DIV', {
				props: {className: 'bx-soa-pp-company feedback-form__left label_payment_block'},
				children: [label],
				events: {
					click: BX.proxy(this.selectPaySystem, this)
				}
		...
		selectDeliveryGroupAction: function(deliveryGroup, change, unsetDelivery)
		{
			...
			$("#bx-soa-delivery .bx-soa-pp-item-container .bx-soa-pp-company").hide();
			$("#bx-soa-delivery .bx-soa-pp-item-container .bx-soa-pp-company." + deliveryGroup).show();
			
			
			if (change) $("#bx-soa-delivery .bx-soa-pp-item-container .bx-soa-pp-company." + deliveryGroup + ' .radio__wrap input').parent().trigger('click');
		},
		
		deliveryGroupID: function(id) {
		for (var key in this.result.DELIVERY_GROUPS) {	
			if (this.deliveryTypes[key].indexOf(parseInt(id)) !== -1) return key;
		}
		},
		...
		editActiveDeliveryBlock: function(activeNodeMode)
		{
			var node = activeNodeMode ? this.deliveryBlockNode : this.deliveryHiddenBlockNode,
				deliveryContent, deliveryNode, regionNode, regionNodeCol, deliveryGroups, deliveryAddress, delivery_groups = [];
				
				regionNode = BX.create('DIV', {props: {className: 'bx_soa_location row'}});
				regionNodeCol = BX.create('DIV', {props: {className: 'col-xs-12'}});
				
				/*this.result.DELIVERY_GROUPS.delivery_pickup = this.result.DELIVERY_GROUPS.delivery_pickup.map(Number);
				
				this.deliveryTypes = {
						delivery_pickup: [3, 23, 25, 29, 33],					
						delivery_delivery: [2, 28, 31],					
						delivery_transport: [19, 20, 21, 24],
				}*/
					
				
				for (var key in this.result.DELIVERY_GROUPS) {
					this.result.DELIVERY_GROUPS[key] = this.result.DELIVERY_GROUPS[key].map(Number);
				}
				
				this.deliveryTypes = this.result.DELIVERY_GROUPS;
			...
			else
			{
				...
				deliveryContent.appendChild(regionNodeCol);
				deliveryContent.appendChild(regionNode);
				...
				var deliveryNames = {
						...
						delivery_delivery: 'Доставкой',					
						delivery_transport: 'Транспортной компанией',					
				}
				...
				deliveryGroups = BX.create('UL', {props: {id: 'select_delivery_group'},
				children: delivery_groups});
				
				
				deliveryContent.appendChild(deliveryGroups);
			...
			this.selectDeliveryGroupAction(this.deliveryGroupID(this.currentDelivery));
		...
		editAddressOther: function() {
			var address = "";
			$("#soa-property-7, #soa-property-19").val($('#address_other').val());
		},		
		...
		editDeliveryItems: function(deliveryNode)
		{
			...
			for (k = 0; k < this.deliveryPagination.currentPage.length; k++)
			{ 	
				if (k==0) this.currentDelivery = this.deliveryPagination.currentPage[k].ID;
				if (this.deliveryPagination.currentPage[k].CHECKED) this.currentDelivery = this.deliveryPagination.currentPage[k].ID; 
				deliveryItemNode = this.createDeliveryItem(this.deliveryPagination.currentPage[k]);
				deliveryItemsContainer.appendChild(deliveryItemNode);
			}
			...
			if (this.deliveryGroupID(this.currentDelivery)=='delivery_delivery' || this.currentDelivery==20 || this.currentDelivery==24) { //Если выбрана доставка по адресу
				deliveryAddress = BX.create('div', {props: {id: 'delivery_address'},
				children: [
					BX.create('DIV', {
									attrs: {},
									props: {
										className: 'bx-soa-customer-field',
									},
									children: [
						BX.create('LABEL', {
							props: {for: 'address_street', className: 'bx-soa-custom-label'},
							children: [
								'Улица',							
							],
						}),
					...
					BX.create('DIV', {
									attrs: {},
									props: {
										className: 'bx-soa-customer-field left',
									},
									children: [
						BX.create('LABEL', {
							props: {for: 'address_house', className: 'bx-soa-custom-label'},
							children: [
								'Дом',							
							],
						}),
					...
					BX.create('DIV', {
									...
									children: [
						BX.create('LABEL', {
							props: {for: 'address_flat', className: 'bx-soa-custom-label'},
							children: [
								'Квартира или офис',							
							],
						}),
...
deliveryAddress = BX.create('div', {props: {id: 'delivery_other_address'},
				children: [
					BX.create('DIV', {
									...
									children: [
						BX.create('LABEL', {
							props: {for: 'address_other', className: 'bx-soa-custom-label'},
							children: [
								'Адрес',							
							],
						}),
			...
			if (this.deliveryGroupID(this.currentDelivery)=='delivery_pickup') {
				deliveryNode.appendChild(this.deliveryMapContainer);
			}
		},
		
		closeMap: function(id) {
			this.addScheme.id.close();
		},
		
		viewMap: function(id) {
			this.addScheme.id = new BX.PopupWindow("scheme" + id, null, {
		content: BX('scheme_text' + id),
		...
		this.addScheme.id.show();
		
		//BX.bind(BX("scheme" + id), 'click', function() {addScheme.id.close();})
		},
		...
		editDeliveryInfo: function(deliveryNode)
		{
			...
			if (currentDelivery.STORE_ADDRESS) 
				storeAdress = BX.create('DIV', {props: {className: 'store_address'}, 
			children: [
				BX.create('DIV', {props: {className: ''}, html: currentDelivery.STORE_ADDRESS
			+ (currentDelivery.STORE_SCHEDULE ? '<br /><b>Время работы:</b> ' + currentDelivery.STORE_SCHEDULE : '')
			+ (currentDelivery.STORE_PHONE ? '<br /><b>Телефон:</b> ' + currentDelivery.STORE_PHONE : '')	
			}),
			currentDelivery.STORE_IMAGE ? BX.create('A', {props: {className: 'scheme', id: 'click_scheme' + currentDelivery.ID, href: 'javascript:void(0)'},
				text: 'Схема проезда',
				events: {click: this.viewMap.bind(this, currentDelivery.ID)}				
			}) : '',	
currentDelivery.STORE_IMAGE ? BX.create('DIV', {props: {className: 'scheme_text d-none', id: 'scheme_text' + currentDelivery.ID}, 
			children: [			
				BX.create('IMG', {props: {className: 'scheme', src: currentDelivery.STORE_IMAGE.SRC, alt: currentDelivery.NAME}}),
				currentDelivery.STORE_DESCRIPTION ? BX.create('DIV', {props: {className: 'description'}, text: currentDelivery.STORE_DESCRIPTION}) : ''
],
events: {click: this.closeMap.bind(this, currentDelivery.ID)}
}) : '',				
			]
			});
...
if (logotype && logotype.src_2x) {
			...
			title = BX.create('DIV', {
				...
				children: [
					...
					currentDelivery.CALCULATE_DESCRIPTION
						...
						: null,
				this.currentDelivery=='!24' ? BX.create('div', {props: {id: 'delivery_other_address'},
				children: [
					BX.create('DIV', {
									attrs: {},
									props: {
						...
						BX.create('LABEL', {
							...
							BX.create('DIV', {
									...
									children: [
										...
										events: {input: this.editAddressOther.bind(this), keyup: this.editAddressOther.bind(this)}
										}),
									],
								}),		
									],
			...
			deliveryInfoContainer.appendChild(
				BX.create('DIV', {
					props: {className: 'bx-soa-pp-company'},
					children: [subTitle, storeAdress, label, title, clear, extraServicesNode, infoList]
				})
			);
...
var deliveryGroupsChange = {};
	
for (var key in this.result.DELIVERY_GROUPS) {	

	deliveryGroupsChange[key] = '';
	
			//Проверка, если ли доставки в группе
			$('div.' + key).each(function(i, elem) {
				if ($(elem).is(':visible')) deliveryGroupsChange[key] = 1;
			});
			if (!deliveryGroupsChange[key]) $("#" + key).hide();	
}			
		...
		{//console.log(item);
			var checked = item.CHECKED == 'Y',
				...
				labelNodes = [
				BX.create('DIV', {
						props: {id: item.CURRENT_STORE ? 'store_item-' + item.CURRENT_STORE : '', className: 'radio__wrap' + (item.CURRENT_STORE ? " store_item" : "")},
				children: [
					BX.create('INPUT', {
						props: {
			...
			logoNode = BX.create('SPAN', {props: {className: 'radio__text'}, html: item.STORE_ADDRESS ? item.STORE_ADDRESS + (item.STORE_SCHEDULE ? '<span><b></b>' + item.STORE_SCHEDULE + '</span>' : '') : (this.params.SHOW_DELIVERY_PARENT_NAMES != 'N' ? item.NAME : item.OWN_NAME)});
			logotype = this.getImageSources(item, 'LOGOTIP');

			labelNodes.push(logoNode);
			
			if (checked) {
				this.currentStore = item.CURRENT_STORE;
			}
			

			label = BX.create('LABEL', {
				props: {
					id: 'ID_DELIVERY_LABEL_' + deliveryId,
					className: 'form__label'
					+ (item.CHECKED == 'Y' ? " active" : "")
			...
			itemNode = BX.create('DIV', {
				props: {className: 'bx-soa-pp-company label_block ' + item.GROUP + (item.CHECKED == 'Y' ? " active" : "")},
				children: [label, title],
		...
		getSelectedPickUp: function()
		{
			...
			if (pickUpInput)
			{
				...
				if (!currentPickUp)
				{
					//stores = [$('.store_item:first').attr("id").replace(/[^0-9]+/, '')];
					if (!stores) stores = this.getSelectedDelivery().STORE;
					...
					{ 
						for (i in stores)
						{
							if (stores.hasOwnProperty(i))
							{
								...
								pickUpInput.setAttribute('value', this.currentStore ? this.currentStore : stores[i]);
								break;
							}
						}
					}
	...
	this.currentStore = $('input[name=DELIVERY_ID]:checked').parent().attr('id').replace(/[^0-9]+/, '');
	this.currentStore = parseInt(this.currentStore);

				storeNode = this.createPickUpItem(currentStore, {selected: currentStore.ID == (this.currentStore ? this.currentStore : selectedStore)});
		...
		pickUpFinalAction: function()
		{
			...
			//var currentstore = $('input[name=DELIVERY_ID]:checked').parent().attr('id').replace(/[^0-9]+/, '');//Не работает
			//if (currentstore) $("#store-" + currentstore).trigger('click');
		},
		...
		editPropsItems: function(propsNode)
		{
			...
			var propsItemsContainer = BX.create('DIV', {props: {className: 'col-sm-12 bx-soa-customer'}}),
				group, property, groupIterator = this.propertyCollection.getGroupIterator(), propsIterator, regionContent, regionNodeCol, regionNode;
				
				var node = this.regionBlockNode;
				...
				regionNodeCol = BX.create('DIV', {props: {className: 'col-xs-12 usertype'}});
				regionNode = BX.create('DIV', {props: {className: 'bx_soa_location row'}});
			...
			propsNode.appendChild(regionContent);	
