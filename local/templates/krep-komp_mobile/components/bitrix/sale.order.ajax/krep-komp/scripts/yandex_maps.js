...
(function() {
	...
	BX.Sale.OrderAjaxComponent.Maps = {
						...
						'<br /><a class="btn btn-sm btn-default" data-store="{{ properties.id }}">{{ properties.text }}</a>',
						{
							build: function() {
								...
								if (button)
									...
									BX.bind(button, 'click', this.selectDeliveryByClick);
							},
							clear: function() {
								var button = document.querySelector('a[data-store]');
								if (button)
									//BX.unbind(button, 'click', this.selectStoreByClick);
									BX.unbind(button, 'click', this.selectDeliveryByClick);

								this.constructor.superclass.clear.call(this);
							},
							selectDeliveryByClick: function(e) {
								//var current_devivery = $('input[name=DELIVERY_ID]:checked').parent().attr('id').replace(/[^0-9]+/, '');
								var current_store = e.target.getAttribute('data-store');
								if (current_store) $('#store_item-' + current_store).parent().trigger('click');
							},
		...
		{
			//alert('(' + storeItemId +')' + typeof(storeItemId));;
