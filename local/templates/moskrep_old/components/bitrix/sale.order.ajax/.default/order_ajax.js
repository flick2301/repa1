	...
	BX.Sale.OrderAjaxComponent = {
		...
		editActivePropsBlock: function(activeNodeMode)
		{
			...
			else
			{
				...
				propsNode = BX.create('DIV', {props: {className: 'row_order feedback-form'}});
				selectedDelivery = this.getSelectedDelivery();
		...
		editPropsItems: function(propsNode)
		{
			...
			var propsItemsContainer = BX.create('DIV', {props: {className: 'feedback-form__left'}}),
		...
		getPropertyRowNode: function(property, propsItemsContainer, disabled)
		{
			...
			else
			{
				...
				if (property.isRequired())
					star = '<span class="bx-authform-starrequired">*</span> ';
                                else
                                    star = '';
                                    

				textHtml += '<span class="feedback-form__title">'+star+BX.util.htmlspecialchars(property.getName())+'</span>';
				...
				label = BX.create('LABEL', {
					...
					props: {className: 'feedback-form__label'},
					html: textHtml
				});
				...
				propsItemNode.appendChild(label);
                                ...
                                var fields = BX.findChild(propsItemNode, {tag: 'div', className: 'soa-property-container'}, true, true);
                                BX.addClass(fields, 'class-name');
		...
		insertYNProperty: function(property, propsItemNode, disabled)
		{
			...
			{
				propContainer = BX.create('DIV', {props: {className: 'soa-property-container1'}});
				property.appendTo(propContainer);
				propsItemNode.appendChild(propContainer);
				this.alterProperty(property.getSettings(), propContainer);
				this.bindValidation(property.getId(), propContainer);
		...
		alterProperty: function(settings, propContainer)
		{
			...
			for (i = 0; i < inputs.length; i++)
			{
				...
				BX.addClass(inputs[i], 'form-control bx-soa-customer-input bx-ios-fix');
                                BX.addClass(divs[i], 'form-control bx-soa-customer-input bx-ios-fix');
			...
			if (add.length)
			{
				...
				BX.bind(add, 'click', BX.delegate(function(e){
					var target = e.target || e.srcElement,
						targetContainer = BX.findParent(target, {tagName: 'label', className: 'feedback-form__label'}),
						del = targetContainer.querySelector('div'),
						...
						divs = BX.findChildren(targetContainer, {tagName: 'LABEL'});
		...
		editPropsComment: function(propsNode)
		{
			...
			propsCommentContainer = BX.create('DIV', {props: {className: 'feedback-form__right'}});
			label = BX.create('LABEL', {
