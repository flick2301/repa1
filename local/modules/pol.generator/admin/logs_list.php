...
if($lAdmin->EditAction())
{
	foreach($FIELDS as $ID=>$arFields)
	{
		...
		$DB->StartTransaction();
		$ID = IntVal($ID);
		$rsData = Pol\Generator\Logs\DB::getById($ID);
		if($arData = $rsData->Fetch())
		{
			...
			$res = Pol\Generator\Logs\DB::update($ID, $arData);
			if(!$res->isSuccess())
			{
...
if(($arID = $lAdmin->GroupAction()))
{
	...
	{
		$rsData = Pol\Generator\Logs\DB::getList($listParams);
...
$columns = Pol\Generator\Logs\LogsTable::getMap();

//IIIIIIIIIIIIIIIIIIIIIIIIIIIIIIIIIIIIIIIIIIIIIIIIIIIIIIIIIIIIIIIIIIIIII
$defaultHeaders = array("ID" => 100, "IBLOCK_ID"=>50, "ID_SECTION"=>200, "ID_KEY"=>300, "ID_RULE"=>400);
$headers = array();
...
while($arRes = $rsData->NavNext(true, "f_")):
	...
	$arActions[] = array(
		...
		"DEFAULT"=>true,
		"TEXT"=> "Изменить",
		"ACTION"=>$lAdmin->ActionRedirect(Pol\Generator\Data::getPageName('edit_log')."?ID=".$f_ID.'&lang='.LANG)
	);
