...
if($lAdmin->EditAction())
{
	foreach($FIELDS as $ID=>$arFields)
	{
		...
		$DB->StartTransaction();
		$ID = IntVal($ID);
		$rsData = Pol\Generator\Key\DB::getById($ID);
		if($arData = $rsData->Fetch())
		{
			...
			$res = Pol\Generator\Key\DB::update($ID, $arData);
			if(!$res->isSuccess())
			{
...
if(($arID = $lAdmin->GroupAction()))
{
	...
	{
		$rsData = Pol\Generator\Key\DB::getList($listParams);
...
$columns = Pol\Generator\Key\KeysTable::getMap();

//IIIIIIIIIIIIIIIIIIIIIIIIIIIIIIIIIIIIIIIIIIIIIIIIIIIIIIIIIIIIIIIIIIIIII
$defaultHeaders = array("ID" => 100,"NAME"=>200, "ID_SECTION"=>300, "KEYS"=>400);
$headers = array();
...
while($arRes = $rsData->NavNext(true, "f_")):
	...
	$arActions[] = array(
		...
		"ACTION"=>$lAdmin->ActionRedirect(Pol\Generator\Data::getPageName('edit_key')."?ID=".$f_ID.'&lang='.LANG)
	);
