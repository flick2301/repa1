...
<?php 
	foreach ($arResult["result"] AS $val) {
		$text[]="<tr onclick=\"window.open('{$val['URL']}', '_blank');\"><td class='search_result_start'><img alt='{$val['NAME']}' src='{$val['PICTURE']}' /></td><td class='search_result_end'><b>{$val['NAME']}</b>".($arParams["ARTNO"] && $val['ART'] ? " (".Loc::getMessage("DSEARCH_ARTNO").": ".trim($val['ART']).")" : "")."<br />
