...
<?foreach($arResult["ITEMS"] AS $key=>$item):?>
                  ...
                  <div class="shops__box" data-rel="<?=$item["ID"]?>">
                    ...
                    <div class="shops__time"><?=$item["PREVIEW_TEXT"]?></div><a class="shops__email" href="mailto:<?=$item["PROP"]["EMAIL"]["VALUE"]?>"><?=$item["PROP"]["EMAIL"]["VALUE"]?></a><a class="shops__link" href="<?=$APPLICATION->GetCurPageParam("ID=".$item["ID"])?>">Перейти к <?if($item["PROP"]["TYPE"]["VALUE_ENUM_ID"]==581):?>точке выдачи<?else:?>магазину<?endif?></a>	
<script>
