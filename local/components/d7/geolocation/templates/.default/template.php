...
<?foreach($arResult["ITEMS"] AS $item):?>
<?if($i >= ceil(/*count($arResult["ITEMS"])/*/3)) {$i = 0; echo "</ul><ul>";}?>
<?if($item["PROP"]["VISIBLE"]["VALUE"]):?><?$i++?><li class="<?if($item["SORT"] < 500):?>master<?endif?>" rel='<?=$item["PROP"]["NAME"]["VALUE"] ? $item["PROP"]["NAME"]["VALUE"] : $item["NAME"]?>' data-value='<?=$item["ID"]?>' data-domain='<?=$item["PROP"]["DOMAIN"]["VALUE"]?>'><?=$item["NAME"]?><?if($item["PROP"]["REGION"]["VALUE"]):?><span><?=$item["PROP"]["REGION"]["VALUE"]?></span><?endif?></li><?endif?>
<?endforeach?>
