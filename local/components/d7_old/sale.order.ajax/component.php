				...
				//{
					...
					if ($bErrorField){
						$arResult["ERROR"][] = GetMessage("SOA_ERROR_REQUIRE")." \"".$arOrderProps["NAME"]."\"";
                                                $arResult["ERROR_FULL"][] = $arOrderProps;
                                        }
			...
			if(!$USER->IsAuthorized() && $arParams["ALLOW_AUTO_REGISTER"] == "Y")
			{
				if(strlen($arUserResult["USER_EMAIL"]) > 0)
				{
					...
					$dbUserLogin = CUser::GetByLogin($NEW_LOGIN);
                                        ...
                                        //Свой код, проверяем EMAIL. Если такой был авторизуем без перерегистрации
                                        $res = CUser::GetList(($by="id"), ($order="asc"), array('EMAIL'=>$arUserResult["USER_EMAIL"]));
                                        $login = $res->fetch();
                                        $arGroups = CUser::GetUserGroup($login['ID']);
                                        if($login && !in_array('1', $arGroups) && !in_array('8', $arGroups)){
