...
<div class="content-feedback basket" id='paysystem_form'>
	<script type="text/javascript">
		...
		<h2 class='s28-title'><?=GetMessage("SOA_TEMPL_PAY_SYSTEM")?></h2>
		<?
		...
		foreach($arResult["PAY_SYSTEM"] as $arPaySystem)
		{
			...
			if ((strlen(trim(str_replace("<br />", "", $arPaySystem["DESCRIPTION"]))) > 0 || intval($arPaySystem["PRICE"]) > 0))
			{
				...
				{
					?>
                                        <li class="pay-choice__item pay-choice--<?=$arPaySystem['DESCRIPTION']?>">
                                            <a href="javascript:void(0);" for="ID_PAY_SYSTEM_ID_<?=$arPaySystem["ID"]?>" onclick="BX('ID_PAY_SYSTEM_ID_<?=$arPaySystem["ID"]?>').checked=true;changePaySystem();" class="pay-choice__link<?echo ($arPaySystem["CHECKED"]=="Y") ? " active" : "";?>"><?=$arPaySystem["PSA_NAME"];?></a>
                                        </li>
					<div class="bx_block w100 vertical">
						<div class="bx_element">
					...
					<div class="bx_block w100 vertical">
						<div class="bx_element">
							...
							<input style='' type="radio"
								...
								value="<?=$arPaySystem["ID"]?>"
								<?if ($arPaySystem["CHECKED"]=="Y" && !($arParams["ONLY_FULL_PAY_FROM_ACCOUNT"] == "Y" && $arResult["USER_VALS"]["PAY_CURRENT_ACCOUNT"]=="Y")) echo " checked=\"checked\"";?>
								onclick="changePaySystem();" />							<div class="clear"></div>
						</div>
					</div>
				<?
				}
