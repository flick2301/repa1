    ...
    <?=bitrix_sessid_post()?>
        ...
        <div class="send-a-request__block send-a-request__block--primary">
            <div class="send-a-request__item">
				...
				<input type="text" value="<?=$arResult["AUTHOR_NAME"]?>" name="user_name" placeholder="<?=GetMessage("MFT_NAME")?>" class="simple-input send-a-request__input">
            </div>
            <div class="send-a-request__item">
                <label class="send-a-request__label" for="send-a-request__phone"></label>
				<input type="text" name="user_tell" placeholder="<?=GetMessage("MFT_TELL")?>" value="<?=$arResult["AUTHOR_TELL"]?>" class="simple-input send-a-request__input">
            </div>
            <div class="send-a-request__item">
                <label class="send-a-request__label" for="send-a-request__email"></label>
				<input type="text" name="user_email" placeholder="<?=GetMessage("MFT_EMAIL")?>" value="<?=$arResult["AUTHOR_EMAIL"]?>" class="simple-input send-a-request__input">
            </div>
			
            <div class="send-a-request__item">
                <label class="send-a-request__label" for="send-a-request__text"></label>
				<textarea name="MESSAGE" placeholder="<?=GetMessage("MFT_MESSAGE")?>" class="simple-textarea send-a-request__textarea"><?=$arResult["MESSAGE"]?></textarea>
            </div>			
		</div>
...
<div class="send-a-request__title">Просто оставьте заявку</div>
	...
	<div class="send-a-request__footer">
            <input id='<?echo ($arParams['HEADER_FORM']=='Y') ? 'header_' : '';?>user_file' name="UPLOAD_FILES" type="file">
          <!--  <a href="javascript:void(0)" onclick="$('#<?echo ($arParams['HEADER_FORM']=='Y') ? 'header_' : '';?>user_file').click();" class="white-btn" style="visibility: hidden">Приложенный файл</a>-->
            <a href="javascript:void(0)" onclick="$('#<?echo ($arParams['HEADER_FORM']=='Y') ? 'header_feedback_form' : 'feedback_form';?>').submit();" class="blue-btn main-button send-a-request__submit">Оставить заявку</a>
	</div>	
