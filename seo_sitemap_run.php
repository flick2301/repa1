...
//							remove or replace SERVER_NAME
							...
							Bitrix\Main\Diag\Debug::dumpToFile("https://krep-komp.ru".$url, "", '/upload/ns.txt');
				...
				else
				{
                    ...
                    $filter = new \CatalogHelpers\FilterButtonsBuilder('section', $arResult, $arSection['ID']);
                    foreach($filter->arResult["SORTING"]["SECTIONS"] as $section)
                    {
                            ...
                            elseif ($arSection['IBLOCK_SECTION_ID'] > 0)
                            {
                                $dbRes = SitemapRuntimeTable::getList(array(
                                    'filter' => array(
                                        ...
                                        'ITEM_ID' => $item['IBLOCK_SECTION_ID'],
                                        'PROCESSED' => SitemapRuntimeTable::PROCESSED,
                                    ),
...
//							remove or replace SERVER_NAME
                                if(!empty($item["sef_filter"]["VALUE"]))
                                    $url = SitemapIblock::prepareUrlToReplace($arSection['SECTION_PAGE_URL'].$item["sef_filter"]["VALUE"].'/', $arSitemap['SITE_ID']);
                                elseif(!$item["arFilters"]["VALUE"])
                                    $url = SitemapIblock::prepareUrlToReplace($arSection['SECTION_PAGE_URL'].$item['CODE'].'/', $arSitemap['SITE_ID']);
