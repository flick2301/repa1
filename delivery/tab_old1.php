<?if($arParams["MAP"]=="Y"):?><script>document.addEventListener("DOMContentLoaded", ready);</script><?endif?>

<p class='info-paragraph' style='color:#f39101;'>Получить груз можно на следующий день. Для этого оформить заказ нужно до 16:00. Суббота и Воскресенье - выходные дни. Бесплатная доставка в пределах МКАД при заказе от 50000 р. Доставка за пределы МКАД оплачивается по тарифной сетке.</p>

			<h2 class="s28-title">Тарифы доставки по городу Москва и МО</h2>
			<table class="blue-table">
				<thead class="blue-table__thead">
					<tr class="blue-table__tr">
						<th class="blue-table__th">Масса груза</th>
						<th class="blue-table__th">Зона - 1</th>
						<th class="blue-table__th">Зона - 2</th>
						<th class="blue-table__th">Зона - 3</th>
						<th class="blue-table__th">Зона - 4</th>
						<th class="blue-table__th">Зона - 5</th>
						<th class="blue-table__th">Зона - 6</th>
						<th class="blue-table__th">Зона - 7</th>						
					</tr>
				</thead>
				<tbody class="blue-table__tbody">
					<tr class="blue-table__tr">
						<td class="blue-table__td">0 - 15 кг</td>
						<td class="blue-table__td">290</td>
						<td class="blue-table__td">490</td>
						<td class="blue-table__td">800</td>
						<td class="blue-table__td">1 300</td>
						<td class="blue-table__td">1 900</td>
						<td class="blue-table__td">3 600</td>
						<td class="blue-table__td">5 200</td>						
					</tr>
					<tr class="blue-table__tr">
						<td class="blue-table__td">15 - 30 кг</td>
						<td class="blue-table__td">350</td>
						<td class="blue-table__td">590</td>
						<td class="blue-table__td">1 000</td>
						<td class="blue-table__td">1 600</td>
						<td class="blue-table__td">2 300</td>
						<td class="blue-table__td">4 500</td>
						<td class="blue-table__td">6 500</td>						
					</tr>
					<tr class="blue-table__tr">
						<td class="blue-table__td">30 - 100 кг</td>
						<td class="blue-table__td">650</td>
						<td class="blue-table__td">900</td>
						<td class="blue-table__td">1 300</td>
						<td class="blue-table__td">1 900</td>
						<td class="blue-table__td">2 700</td>
						<td class="blue-table__td">5 200</td>
						<td class="blue-table__td">7 200</td>							
					</tr>
					<tr class="blue-table__tr">
						<td class="blue-table__td">100 - 200 кг</td>
						<td class="blue-table__td">1 000</td>
						<td class="blue-table__td">1 300</td>
						<td class="blue-table__td">1 700</td>
						<td class="blue-table__td">2 300</td>
						<td class="blue-table__td">3 000</td>
						<td class="blue-table__td">6 000</td>
						<td class="blue-table__td">8 000</td>							
					</tr>
					<tr class="blue-table__tr">
						<td class="blue-table__td">200 - 500 кг</td>
						<td class="blue-table__td">1 950</td>
						<td class="blue-table__td">2 250</td>
						<td class="blue-table__td">2 600</td>
						<td class="blue-table__td">3 250</td>
						<td class="blue-table__td">4 000</td>
						<td class="blue-table__td">7 500</td>
						<td class="blue-table__td">9 000</td>							
					</tr>
					<tr class="blue-table__tr">
						<td class="blue-table__td">500 - 1 000 кг</td>
						<td class="blue-table__td">2 600</td>
						<td class="blue-table__td">2 900</td>
						<td class="blue-table__td">3 300</td>
						<td class="blue-table__td">3 900</td>
						<td class="blue-table__td">4 700</td>
						<td class="blue-table__td">9 000</td>
						<td class="blue-table__td">11 000</td>							
					</tr>
					<tr class="blue-table__tr">
						<td class="blue-table__td">1 000 - 3 000 кг</td>
						<td class="blue-table__td">2 900</td>
						<td class="blue-table__td">3 300</td>
						<td class="blue-table__td">3 800</td>
						<td class="blue-table__td">4 500</td>
						<td class="blue-table__td">5 500</td>
						<td class="blue-table__td">10 000</td>
						<td class="blue-table__td">12 000</td>							
					</tr>
					<tr class="blue-table__tr">
						<td class="blue-table__td">3 000 - 5 000 кг</td>
						<td class="blue-table__td">3 900</td>
						<td class="blue-table__td">4 500</td>
						<td class="blue-table__td">5 200</td>
						<td class="blue-table__td">6 100</td>
						<td class="blue-table__td">6 900</td>
						<td class="blue-table__td">12 000</td>
						<td class="blue-table__td">15 000</td>							
					</tr>
                    <tr class="blue-table__tr">
						<td class="blue-table__td">5 000 - 10 000 кг</td>
						<td class="blue-table__td">5 900</td>
						<td class="blue-table__td">6 300</td>
						<td class="blue-table__td">6 800</td>
						<td class="blue-table__td">7 500</td>
						<td class="blue-table__td">8 500</td>
						<td class="blue-table__td">15 000</td>
						<td class="blue-table__td">19 000</td>							
					</tr>
				</tbody>
			</table>
			<p class="info-paragraph">* - выгрузка товара, до 30 кг - 300 руб. на лифте, и 100 руб. за этаж, если по лестнице.</p>
			
					<h2 class="s28-title">Тариф доставки "День в день"<br />(в пределах МКАД)</h2>
			<table class="blue-table" style="display: table; min-width: 300px;">
				<thead class="blue-table__thead">
					<tr class="blue-table__tr">
						<th class="blue-table__th" style="width: 50%;">Масса груза</th>
						<th class="blue-table__th" style="width: 50%;">Зона - 1</th>					
					</tr>
				</thead>
				<tbody class="blue-table__tbody">
					<tr class="blue-table__tr">
						<td class="blue-table__td" style="width: 50%;">0 - 15 кг</td>
						<td class="blue-table__td" style="width: 50%;">350</td>					
					</tr>
					<tr class="blue-table__tr">
						<td class="blue-table__td" style="width: 50%;">15 - 30 кг</td>
						<td class="blue-table__td" style="width: 50%;">450</td>					
					</tr>
					<tr class="blue-table__tr">
						<td class="blue-table__td" style="width: 50%;">30 - 100 кг</td>
						<td class="blue-table__td" style="width: 50%;">800</td>					
					</tr>
					<tr class="blue-table__tr">
						<td class="blue-table__td" style="width: 50%;">100 - 200 кг</td>
						<td class="blue-table__td" style="width: 50%;">1 500</td>					
					</tr>
					<tr class="blue-table__tr">
						<td class="blue-table__td" style="width: 50%;">200 - 500 кг</td>
						<td class="blue-table__td" style="width: 50%;">2 500</td>					
					</tr>
					<tr class="blue-table__tr">
						<td class="blue-table__td" style="width: 50%;">500 - 1 000 кг</td>
						<td class="blue-table__td" style="width: 50%;">3 900</td>					
					</tr>
					<tr class="blue-table__tr">
						<td class="blue-table__td" style="width: 50%;">1 000 - 3 000 кг</td>
						<td class="blue-table__td" style="width: 50%;">4 500</td>					
					</tr>
					<tr class="blue-table__tr">
						<td class="blue-table__td" style="width: 50%;">3 000 - 5 000 кг</td>
						<td class="blue-table__td" style="width: 50%;">5 500</td>					
					</tr>
					<tr class="blue-table__tr">
						<td class="blue-table__td" style="width: 50%;">5 000 - 10 000 кг</td>
						<td class="blue-table__td" style="width: 50%;">9 000</td>					
					</tr>					
				</tbody>
			</table>				
		

							<h2 class="s28-title">Тариф доставки "Доставка по субботам"<br />(в пределах МКАД)</h2>
			<table class="blue-table" style="display: table; min-width: 300px;">
				<thead class="blue-table__thead">
					<tr class="blue-table__tr">
						<th class="blue-table__th" style="width: 50%;">Масса груза</th>
						<th class="blue-table__th" style="width: 50%;">Зона - 1</th>					
					</tr>
				</thead>
				<tbody class="blue-table__tbody">
					<tr class="blue-table__tr">
						<td class="blue-table__td" style="width: 50%;">0 - 15 кг</td>
						<td class="blue-table__td" style="width: 50%;">350</td>					
					</tr>
					<tr class="blue-table__tr">
						<td class="blue-table__td" style="width: 50%;">15 - 30 кг</td>
						<td class="blue-table__td" style="width: 50%;">450</td>					
					</tr>
					<tr class="blue-table__tr">
						<td class="blue-table__td" style="width: 50%;">30 - 100 кг</td>
						<td class="blue-table__td" style="width: 50%;">800</td>					
					</tr>
					<tr class="blue-table__tr">
						<td class="blue-table__td" style="width: 50%;">100 - 300 кг</td>
						<td class="blue-table__td" style="width: 50%;">2 300</td>					
					</tr>
				</tbody>
			</table><br /><br />				
		

        <div id="map" style="width: 100%;height: 500px;" class="mb-30"></div>
		
